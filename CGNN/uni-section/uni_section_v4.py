...
20260401_yj.py 의 고급 손실함수 체계(Smoothness-angle, Mass, Collision v3)와
커리큘럼 학습 스케줄러를 이식한 버전.

docs/command_v3.md 지시서 반영 사항:
  - Step 1: compute_smoothness_loss_angle / compute_mass_loss /
            compute_collision_loss_v3 이식
            (compute_repulsive_keepout_loss 는 이번 단계에서 제외 — 설계 제한 영역 미설정)
  - Step 2: get_curriculum_weights 이식 (단일 섹션이므로 continuity/shape 항 제외,
            keepout 항도 제외 → s_phys/s_smooth/s_mass/s_collision 4개만 반환)
  - Step 3: weights 딕셔너리 기반 train_step 재구성 (w_keepout 없음)
  - Step 4: keepout 관련 인자/함수는 시그니처에서 완전히 제거 (v3 대비 차이점)
  - l_phys 형태는 20260401_yj.py 원본과 동일하게 sqrt(abs(err_ratio)) 유지
    (command_v3.md §5 "phys loss 형태 정합성" 반영)
  - compute_section_continuity_loss / compute_shape_continuity_loss 는
    층간(섹션간) 연속성을 위한 함수이므로 단일 섹션 모델에서는 제외 (v3와 동일)
"""
...
# ══════════════════════════════════════════════════════════════════
# SECTION 1: Loss Functions  (20260401_yj.py 에서 이식, command_v3.md Step 1)
# ══════════════════════════════════════════════════════════════════
# 주의:
#  - compute_section_continuity_loss / compute_shape_continuity_loss 는
#    층(섹션)간 연속성을 위한 함수이므로 단일 섹션 모델에서는 이식하지 않음.
#  - compute_repulsive_keepout_loss(설계 제한 영역) 는 command_v3.md 지시에 따라
#    이번 버전에서 이식하지 않음.
...
def get_curriculum_weights(epoch, total_epochs, curriculum_ratio):
    ...
    단일 섹션 모델 + keepout 미사용이므로 continuity/shape/keepout 항은 제외하고
    (s_phys, s_smooth, s_mass, s_collision) 4개만 반환합니다.

    Phase 1 (epoch < total_epochs * curriculum_ratio[0]):
        s_phys=1.0 고정, s_smooth=s_mass=0.0 → 물리(Mp) 수렴에 우선 집중
    Phase 2 (curriculum_ratio[0] ~ curriculum_ratio[1]):
        sine 기반 0→1 ramp (완만 → 급격 증가)
    Phase 3 (curriculum_ratio[1] ~ 1.0):
        모든 활성 항 가중치 1.0 고정
    s_collision은 커리큘럼 대상에서 제외하고 처음부터 1.0 고정
    (간섭은 학습 초반부터도 억제되어야 하는 항이므로).
    """
...
# ══════════════════════════════════════════════════════════════════
# SECTION 4: Training Step  (yj의 다목적 손실 + curriculum 을 단일 섹션에 적용, keepout 제외)
# ══════════════════════════════════════════════════════════════════
...
def train_step(model, data, optimizer, target_mps, target_area,
    ...
    """
    docs/command_v3.md Step 3 반영:
    - weights 딕셔너리 기반 다목적 손실 합산 (w_phys/w_smooth/w_mass/w_collision, keepout 없음)
    - get_curriculum_weights 로 매 에폭 s_* multiplier 적용
    - compute_section_continuity_loss / compute_shape_continuity_loss / keepout 는 제외
    """
    model.train()
    optimizer.zero_grad()
    ...
    ## ── 층별(단일 섹션) 물리 손실 (L_phys) — 20260401_yj.py와 동일한 sqrt(abs(err)) 형태 ──
    ...
    ## ── 다목적 손실 계산 (Step 1 이식 함수, keepout 제외) ──
...
def run_training(data, target_mps, target_area, parts_order_in_sections,
                  ...
                  curriculum_ratio=(0.1, 0.5), snapshot_interval=10):
    """
    pna_solver_validate_v9.py 의 run_training() 구조(snapshot 저장) +
    20260401_yj.py 의 다목적 손실/커리큘럼 train_step 결합 (keepout 제외).
    """
    ...
    print(f"\n{'=' * 65}")
    print(f"[ uni_section_v4 ] Training  |  Target Mp = {target_mps[0]:,.0f} N·mm  |  Epochs: {max_epochs}")
    print(f"  CGDN: hidden=128, layers=4, heads=4  |  Curriculum: {curriculum} {curriculum_ratio}")
    print(f"  (keepout 미사용 — 설계 제한 영역은 이번 버전 범위 외)")
    print(f"{'=' * 65}")
    print(f"Epoch ||  Loss  ||  Phys  |  Smth  |  Area  |  Mass  |  Coll")
...
def visualize_training(history, base_coords, result_coords, target_mp_val, part_labels=None):
    ...
    plt.suptitle('uni_section_v4 학습 결과  |  B-Pillar 5 Parts (keepout 제외)', fontsize=13, fontweight='bold')
    plt.tight_layout()
    try:
    ...
    out_path = os.path.join(out_dir, 'uni_section_v4_result.png')
...
if __name__ == "__main__":
    ...
    print("uni_section_v4: validate_v9 초기 좌표/타겟 Mp + yj 고급 손실함수(phys/smooth/mass/collision)/커리큘럼 학습 통합")
    print("(설계 제한 영역 keepout_loss 항은 이번 버전에서 제외)")
    ...
    ## 계층 순서(Outer→Inner): B-Pillar 좌표상 배치 순서 (v3와 동일하게 유지)
